...
将现有 Earth Explorer 改造成单视口探索工作台：压缩重复的标题与统计信息，让地球画布在目标桌面视口中占据至少 70% 高度；采用 Retro-Futuristic 深空观测站视觉锚点，以深海军蓝、霓虹青和选中洋红替换大面积黑金/黄褐色；保留 `react-globe.gl` 与 Three.js，通过合并国家点云、移除悬停全量重算、保持 HTML 标记节点稳定和可取消的 last-intent-wins 相机动画提升流畅度；用 GeoJSON 边界内缩采样生成确定性国家内部槽位，并在容量不足时聚合多余游戏；以原子探索 reducer 保证国家、区域、游戏和相机意图一致。
...
**Scale/Scope**: 当前约 992 条游戏、21 个本地国家/地区、236 个世界边界 feature、550 个国家点阵点；全球每国 1 个代表标记、区域每国至多 6 个、选中国家至多 12 个候选且受安全槽位容量约束
...
- 顶栏只保留返回、产品名和必要当前状态；删除标题区与地球状态区的重复统计。右栏内部滚动且与地图等高。
...
- 将 Earth Explorer 颜色集中为语义 CSS token；DOM、Three.js 材质、气氛与 CountryLayer 共用同一语义层级。暖色只允许用于评分/警示的小面积语义。
...
- 普通切换动画目标 560–600ms，UI 状态在事件批次内立即更新；减少动态效果或极快速连续输入时可直接跳到最新目标。
...
- 将 550 个国家点阵点合并为单一 points mesh，并把点阵设为非交互装饰；国家交互由 polygon、国家聚合标记和右栏承担。
...
- polygon 曲率、点分辨率和 hover 更新频率以轮廓可读性为约束逐步降低；保留 pixel ratio 1.25、关闭 antialias、轻量 GeoJSON 和 marker cap 等现有正确策略。
...
4. 稳定 marker DOM、合并国家点云、移除 hover 全量计算并清理引擎监听器。
5. 重构桌面单视口布局和控制托盘，随后实施 Retro-Futuristic token、Three.js/CountryLayer 配色和响应式状态。
...
- **合并点云后不能逐点点击**: polygon、国家聚合标记和国家列表继续提供选择，不依赖装饰点阵交互。
